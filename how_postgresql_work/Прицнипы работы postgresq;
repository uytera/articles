...
# Автоматическая очистка таблиц (Autovacuum)
...
При выполнении запросов PostgreSQL далеко не всегда требуется просматривать всю таблицу. Если необходимо найти небольшое количество строк по определенному условию, последовательное чтение каждой страницы становится неэффективным. Для ускорения поиска используются **индексы**.

Индекс представляет собой дополнительную структуру данных, содержащую значения индексируемых столбцов и ссылки на соответствующие строки таблицы. Благодаря этому PostgreSQL может значительно сократить объем данных, которые необходимо прочитать для поиска нужных записей.

Поддержка индексов требует дополнительных ресурсов. При каждой операции `INSERT`, `UPDATE` или `DELETE` PostgreSQL должен поддерживать их актуальность, поэтому наличие индексов ускоряет чтение данных, но увеличивает стоимость операций изменения.

По умолчанию PostgreSQL использует индекс типа **B-Tree**, который подходит для большинства задач и применяется при создании индекса командой `CREATE INDEX`, если тип индекса явно не указан.

### Структура B-Tree

Индекс B-Tree представляет собой сбалансированное дерево поиска.

Внутренние страницы дерева содержат только ключи и ссылки на дочерние страницы, определяя маршрут дальнейшего поиска.
Листовые страницы содержат значения индексируемых столбцов и ссылки на соответствующие кортежи таблицы (TID — Tuple Identifier).
Все листовые страницы располагаются на одном уровне дерева, благодаря чему глубина поиска остается практически одинаковой независимо от значения ключа.

Поиск начинается с корневой страницы. На каждом уровне PostgreSQL сравнивает искомое значение с ключами текущей страницы и выбирает дочернюю страницу, в которой может находиться требуемый ключ.
После достижения листовой страницы PostgreSQL получает соответствующий TID и переходит к чтению страницы Heap для проверки видимости строки в рамках текущей транзакции.

Высота дерева обычно невелика даже для очень больших таблиц, поэтому поиск требует чтения лишь нескольких страниц индекса.

### Tuple Identifier (TID)

Каждая запись индекса содержит **Tuple Identifier (TID)** — физический адрес версии строки в Heap.

TID состоит из двух частей:

* номера страницы Heap;
* номера кортежа внутри страницы.

Используя TID, PostgreSQL может напрямую перейти к нужной странице Heap, не выполняя последовательного поиска строки.

Следует учитывать, что индекс не содержит информации о видимости кортежей. После получения TID сервер обязан обратиться к Heap и проверить, доступна ли найденная версия строки текущей транзакции в соответствии с правилами MVCC.

Именно поэтому обычный **Index Scan** практически всегда сопровождается обращением к данным таблицы.
